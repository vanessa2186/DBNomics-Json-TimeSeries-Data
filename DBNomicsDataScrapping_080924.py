...
def remove_same_value_columns(df):
    
    '''
    Drops those columns in dataframe
    ...
    # Check if all values in col are the same
    for col in df.columns:
        if (df[col] == df[col].iloc[0]).all():
           cols_to_remove.append(col)

    if 'indexed_at' in cols_to_remove:
        cols_to_remove.remove('indexed_at')

    # Drop columns
    df_cols_removed = df.drop(columns=cols_to_remove)
...
time_series = remove_same_value_columns(df_ts)
reference_data = remove_same_value_columns(df_ref_exp)


print("Successfully removed unnessesary columns!")
...
time_series.to_csv('CHELEM_TRADE_CHEL.csv')
reference_data.to_csv('CHELEM_TRADE_CHEL_REFDATA.csv')
